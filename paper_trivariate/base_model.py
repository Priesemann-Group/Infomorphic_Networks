...
class InfomorphicContextLateralModel(BaseModel):
    ...
    def loss(self, gamma=None, return_atoms=False):  
        return self.hidden_layer1.loss(gamma=self.layer_params.hidden_layer1.gamma,return_atoms=return_atoms), self.output_layer.loss(gamma=self.layer_params.output_layer.gamma, return_atoms=return_atoms)
...
def trainer(dataloader, optim, model, device, master_bar, update=True, return_batchwise=False):
    ...
    for i_batch, (x_r, x_c) in enumerate(dataloader): #fastprogress.progress_bar(dataloader, parent=master_bar)):
        ...
        if model.global_opt:
            ...
            tuples = model.loss(return_atoms=True)
            ep_loss = 0
        ...
        # conf_matrix += confusion_matrix(x_c.cpu().numpy(), pred.argmax(dim=1).cpu().numpy(), labels=range(10))
