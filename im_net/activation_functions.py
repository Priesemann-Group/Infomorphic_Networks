...
class LinearTernaryActivation(TernaryActivationFunction):
    def __init__(self, output_size, d=None):
        super().__init__()
        if d is None:
            d = 3 * torch.ones(output_size, requires_grad=True)#5 #* torch.rand(1, requires_grad=True)
        else:
    ...
    def forward(self, r, c):
        x = r+c
        f = torch.sigmoid(-(x+torch.abs(self.d))) # high for small x
        h = torch.sigmoid((x-torch.abs(self.d))) # high for large x
        g = 1-f-h # high for intermediate x
        return torch.stack([f,g,h], dim=2)


class SumActivation(BinaryActivationFunction):
    def __init__(self, output_size, beta=1):
        super().__init__()
        self.beta = beta
    def forward(self, r, c):
        theta = torch.sigmoid(self.beta*(1*r+c))
        return torch.stack([1-theta, theta], dim=2)

class MakkehActivation(BinaryActivationFunction):
    def __init__(self, output_size, beta=1):
        super().__init__()
        self.beta = beta
    def forward(self, r, c):
        theta = torch.sigmoid(self.beta*(c))
        return torch.stack([1-theta, theta], dim=2)
...
class BinaryHeaviside(nn.Module):
    ...
    def forward(self, r,c):
        return HeavisideSigmoidFunction.apply(r+c)
