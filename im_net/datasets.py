...
class HopfieldDataset(CustomDataset):
    def __init__(self, num_samples,num_neurons=100,p_bernoulli=0.5,device='cpu'):
        thetas = p_bernoulli*torch.ones((num_samples,num_neurons))
        self.data = 2*torch.bernoulli(thetas)-1
        self.data = self.data.to(device)
        super().__init__(self.data)

class MarkovHopfieldDataset(CustomDataset):
    def __init__(self, num_samples, num_neurons,spatial,p_bernoulli=0.5,device='cpu'):
        thetas = 2*torch.bernoulli(p_bernoulli*torch.ones((num_samples,num_neurons)))-1
        if spatial:
            thetas.transpose_(0,1)
        thetas[0]=2*torch.bernoulli(0.5*torch.ones(thetas.size(1)))-1
        for i in range(1,thetas.size(0)):
